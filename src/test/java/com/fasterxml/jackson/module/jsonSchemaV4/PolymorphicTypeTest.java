...
public class PolymorphicTypeTest {
    public static final HashSet<String> JSON_SUB_TYPES = new HashSet<>(asList("Person", "Company", "BigCompany"));
...
//        Assert.assertNotNull("Polymorphic object should have an ID", schema.getId());
    ...
    /*
    public static AnyOfSchema getAnyOfFromArray(JsonSchema schema) {
    ...
    */
    ...
    public static void verifyDefinitions(JsonSchema schema) {
        containsDefinitions(schema, new HashSet<>(asList("Person", "Company", "BigCompany", "Company_1", JSONSubTypeBaseClass.class.getSimpleName())));
        ...
        verifyAnyOfContent(companyPolymorphic.asPolymorphicObjectSchema().getAnyOf(), new HashSet<>(asList("Company_1", "BigCompany")));
    ...
    public void testUnionType() throws JsonProcessingException {
        ...
        containsDefinitions(schema, new HashSet<>(asList("Number", "Double", "Integer")));
        ...
        verifyAnyOfContent(number.asPolymorphicObjectSchema().getAnyOf(), new HashSet<>(asList("Integer", "Double")));
        Assert.assertTrue("Number should have an array of types", number.getType().isArrayJSONType());
        Assert.assertEquals("Number has wrong set of types", new HashSet<>(asList(JsonFormatTypes.INTEGER, JsonFormatTypes.NUMBER)), new HashSet<>(asList(number.getType().asArrayJsonType().getFormatTypes())));
    }
}
